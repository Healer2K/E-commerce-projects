...
export default {
  ...
  //当组件挂载完毕之前执行执行一次【先于mounted之前】
  beforeMount() {
    //Object.assign:ES6新增的语法 合并对象
    //在发送请求之前，把接口需要传递参数进行整理（在给服务器发请求之前，把参数整理好，服务器就会返回查询的数据）
  ...
  mounted() {
    //先测试接口返回的数据格式
    //在发请求之前带给服务器参数【search参数发生变化有数值贷给服务器】
    this.getData();
  },
  computed: {
  ...
  methods: {
    //想服务器发生请求获取search模块数据（根据参数不同返回不同的数据进行展示）
    ...
    //排序的操作
    changeOrder(flag){
      //flag形参：它是一个标记，代表用户点击的是综合（1） 还是 价格（2） 用户点击时传递进来的
